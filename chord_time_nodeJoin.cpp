...
Node* Node::findPredecessor(int id)
{
    ...
    while(!isInInterval(id,n_new->id,n_new->successor->id,0,1))
    {
        ...
        if (TIME)
        {
            DISTANCE = DISTANCE + distance(n_prev,n_new);
        }
        n_prev = n_new;
    }
...
Node* Node::findSuccessor(int id)
{
    ...
    if(TIME)
    {
        DISTANCE = DISTANCE + distance(this,n);
    }
    return succ;
}
...
void Node::updateFingerTable(Node* s, int i)
{
                                      ...
                                      s != this)
    {
        ...
        if (TIME)
        {
            DISTANCE = DISTANCE + distance(this,p);
        }
...
int main (void)
{
    ...
    TIME = 1;
    cout << "----ADDING ADDITIONAL " << NODES_ADD << " NODES---------\n";
    double avg_dis = 0;
    for (int i = NODES; i < NODES+NODES_ADD; i++)
    {
        /* code */
        DISTANCE = 0;
        n[i].setValue(rand()%ID_SPACE, LIVE, rand()%GRID, rand()%GRID);
        ...
        avg_dis = avg_dis + DISTANCE/1e06;
    }
    nodeJoin_t.close();

    avg_dis = avg_dis*(1e06/NODES_ADD);

    cout << "AVERAGE TIME FOR ADDITION OF NODES: " << avg_dis << " UNITS\n";
